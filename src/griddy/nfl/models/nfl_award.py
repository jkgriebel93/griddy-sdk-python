...
class NFLAward(BaseModel):
    """
    NFLAward
    """ # noqa: E501
    award_type: Optional[StrictStr] = Field(default=None, alias="awardType")
    description: Optional[StrictStr] = None
    year: Optional[StrictInt] = None
    __properties: ClassVar[List[str]] = ["awardType", "description", "year"]

    @field_validator('award_type')
    def award_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['MVP', 'OPOY', 'DPOY', 'OROY', 'DROY', 'PRO_BOWL', 'ALL_PRO', 'SUPER_BOWL_MVP']):
            raise ValueError("must be one of enum values ('MVP', 'OPOY', 'DPOY', 'OROY', 'DROY', 'PRO_BOWL', 'ALL_PRO', 'SUPER_BOWL_MVP')")
        return value
