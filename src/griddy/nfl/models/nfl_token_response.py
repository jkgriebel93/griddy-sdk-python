...
class NFLTokenResponse(BaseModel):
    """
    NFLTokenResponse
    """ # noqa: E501
    access_token: Optional[StrictStr] = Field(default=None, description="JWT access token containing user permissions, subscription plans, location data, and roles. Include this token in the Authorization header as \"Bearer {accessToken}\" for authenticated requests.", alias="accessToken")
    expires_in: Optional[StrictInt] = Field(default=None, description="Unix timestamp when the access token expires", alias="expiresIn")
    refresh_token: Optional[StrictStr] = Field(default=None, description="New refresh token for future token refresh requests", alias="refreshToken")
