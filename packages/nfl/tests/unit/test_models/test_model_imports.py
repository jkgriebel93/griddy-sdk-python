...
_MODELS_ROOT = Path(__file__).resolve().parents[3] / "src" / "griddy_nfl" / "models"
...
def _discover_model_modules():
    ...
    for subdir in _MODEL_SUBDIRS:
        ...
        for py_file in sorted(subdir_path.glob("*.py")):
            ...
            module_name = f"griddy_nfl.models.{subdir}.{py_file.stem}"
...
_KNOWN_BROKEN = {
    "griddy_nfl.models.responses.coaches_film_response",  # wrong relative import for response_metadata
}
