...
from .conftest import multi_team_players_parser as _parser
...
class TestMultiTeamPlayersSmoke:
    ...
    def test_has_required_keys(self, multi_team_parsed):
        assert_smoke(
            multi_team_parsed, ["title", "teams", "top_players", "stats_tables"]
        )
...
class TestEndpointConfig:
    ...
    def test_endpoint_via_mock(self, multi_team_html):
        result = assert_endpoint_via_mock(
            multi_team_html,
            "get_multi_team_players",
            MultiTeamPlayers,
            method_kwargs={"t1": "crd", "t2": "atl"},
        )
