# coding: utf-8

"""
    NFL REST APIs

    Regular API - NFL's public API for accessing game schedules, team information, standings, statistics, and venue data. This API provides comprehensive access to NFL data including real-time game information, team rosters, seasonal statistics, and historical data. The NFL Pro API is for accessing advanced statistics, film room content, player data, and fantasy information. This API provides comprehensive access to NFL Pro features including Next Gen Stats, Film Room analysis, player projections, and game insights.

    The version of the OpenAPI document: 1.0.0
    Contact: john@thistlegrow.software
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from nfl.models.nfl_team_roster_response import NFLTeamRosterResponse

class TestNFLTeamRosterResponse(unittest.TestCase):
    """NFLTeamRosterResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> NFLTeamRosterResponse:
        """Test NFLTeamRosterResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `NFLTeamRosterResponse`
        """
        model = NFLTeamRosterResponse()
        if include_optional:
            return NFLTeamRosterResponse(
                season = 2025,
                team = nfl.models.team_info.TeamInfo(
                    abbr = 'MIN', 
                    city_state = 'Minnesota', 
                    conference = nfl.models.conference.Conference(
                        abbr = 'AFC', 
                        full_name = 'National Football Conference', 
                        id = '0015', ), 
                    conference_abbr = 'AFC', 
                    division = nfl.models.division.Division(
                        full_name = 'NFC North Division', 
                        id = '0017', ), 
                    full_name = 'Minnesota Vikings', 
                    logo = '', 
                    nick = 'Vikings', 
                    season = 56, 
                    smart_id = '', 
                    stadium_name = '', 
                    team_id = '', 
                    team_site_ticket_url = '', 
                    team_site_url = '', 
                    team_type = 'PRO', 
                    ticket_phone_number = '', 
                    year_found = 56, ),
                team_players = [
                    nfl.models.player.Player(
                        birth_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), 
                        college_conference = 'Southeastern Conference', 
                        college_name = 'Louisiana State', 
                        current_team_id = '', 
                        display_name = 'Justin Jefferson', 
                        draft_club = '', 
                        draft_number = 56, 
                        draftround = 56, 
                        entry_year = 56, 
                        esb_id = '', 
                        first_name = '', 
                        football_name = '', 
                        gsis_id = '', 
                        gsis_it_id = 56, 
                        headshot = '', 
                        height = '6-3', 
                        jersey_number = 56, 
                        last_name = '', 
                        nfl_id = 52430, 
                        ngs_position = null, 
                        ngs_position_group = null, 
                        position = 'C', 
                        position_group = 'DB', 
                        rookie_year = 56, 
                        season = 56, 
                        short_name = 'J.Jefferson', 
                        smart_id = '', 
                        status = 'ACT', 
                        status_description_abbr = 'A01', 
                        status_short_description = 'Active', 
                        team_abbr = '', 
                        uniform_number = '18', 
                        weight = 56, 
                        years_of_experience = 56, )
                    ]
            )
        else:
            return NFLTeamRosterResponse(
        )
        """

    def testNFLTeamRosterResponse(self):
        """Test NFLTeamRosterResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
