# coding: utf-8

"""
    NFL REST APIs

    Regular API - NFL's public API for accessing game schedules, team information, standings, statistics, and venue data. This API provides comprehensive access to NFL data including real-time game information, team rosters, seasonal statistics, and historical data. The NFL Pro API is for accessing advanced statistics, film room content, player data, and fantasy information. This API provides comprehensive access to NFL Pro features including Next Gen Stats, Film Room analysis, player projections, and game insights.

    The version of the OpenAPI document: 1.0.0
    Contact: john@thistlegrow.software
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from nfl.models.nfl_pass_rusher_stats import NFLPassRusherStats

class TestNFLPassRusherStats(unittest.TestCase):
    """NFLPassRusherStats unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> NFLPassRusherStats:
        """Test NFLPassRusherStats
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `NFLPassRusherStats`
        """
        model = NFLPassRusherStats()
        if include_optional:
            return NFLPassRusherStats(
                nfl_id = 34452,
                gsis_id = '00-0039852',
                esb_id = 'NAC559347',
                jersey_number = 9,
                player_name = 'Matthew Stafford',
                short_name = 'M.Stafford',
                team_id = '2510',
                headshot = 'https://static.www.nfl.com/image/upload/formatInstructions/league/oyap81gtzcvnfmripis1',
                position = 'C',
                blitz_count = 37,
                avg_separation_to_qb = 4.797052221914545,
                tackles = 4,
                assists = 2,
                sacks = 1.5,
                forced_fumbles = 1
            )
        else:
            return NFLPassRusherStats(
        )
        """

    def testNFLPassRusherStats(self):
        """Test NFLPassRusherStats"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
